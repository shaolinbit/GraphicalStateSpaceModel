...
for k=1:N-1
    ...
    if k>win_size
        ...
        mean_x(win_size+2,1)=mean(EXk_opt(k,1,:),3)';%给最后一个元素赋值
        mean_y(win_size*2+1:win_size*2+2,1)=[mean(EXk1_opt(k+1,1,:),3)';mean(z_opt(k,:),2)];%给窗口里最后一个历元的x、z赋值
